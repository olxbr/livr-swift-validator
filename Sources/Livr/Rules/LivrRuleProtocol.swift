...
extension LivrRule {
    public func buildError(inputErrors: Any?, inputArgs: Any? = nil) -> Any? {
        if LIVR.isBoundary {
            let errors = (inputErrors as? [Any])?.compactMap { $0 as? [String: Any?] }.last
            return errors ?? ["Errors": inputErrors, "Args": inputArgs]
        }
